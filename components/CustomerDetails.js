...
export default function CustomerDetails({ route, navigation }) {
  const { orders, setOrders, customers, setCustomers, shopId } = useContext(OrderContext);
  ...
  const saveCustomerEdit = async () => {
    ...
    try {
      const oldPhoneKey = (customerPhoneParam || "").replace(/\D/g, "") || Date.now().toString();
      const newPhoneKey = (editPhone || "").replace(/\D/g, "") || Date.now().toString();
      const oldCustomerId = `${userId}_${oldPhoneKey}`;
      const newCustomerId = `${userId}_${newPhoneKey}`;

      // If phone changed, delete old customer and create new
      if (!shopId) {
      ...
      if (oldCustomerId !== newCustomerId) {
        // Delete old customer
        await Promise.all([
          remove(ref(db, `customers/${oldCustomerId}`)),
          remove(ref(db, `users/${userId}/customers/${oldCustomerId}`)),
        ]);
      }

      // Save new/updated customer
      const customerData = {
        id: newCustomerId,
      ...
      await Promise.all([
        set(ref(db, `customers/${newCustomerId}`), customerData),
        set(ref(db, `users/${userId}/customers/${newCustomerId}`), true),
      ]);
      ...
      const ordersToUpdate = orders.filter((o) => o.customerPhone === customerPhoneParam);
      for (const order of ordersToUpdate) {
        const updatedOrder = { ...order, customerName: editName, customerPhone: editPhone };
        await set(ref(db, `orders/${order.id}`), updatedOrder);
      }
 ...
 const deleteCustomer = (customerId) => {
    ...
    { text: "Cancel", style: "cancel" },
    {
      ...
      onPress: async () => {
        try {
          ...
          await Promise.all([
            remove(ref(db, `customers/${compositeCustomerId}`)), // ✅ Use top-level customer path
  ...
  const setStatus = (id, status) => {
    ...
    if (!userId || !orderToUpdate || !shopId) return;
