...
export default function NewOrder({ navigation, route }) {
  ...
  const calculateUnitPrice = (item, companyName) => {
    ...
    if (company) {
      if (name.includes("steel")) {
        const unit = company.steelUnit > 0 ? company.steelUnit : 1000;
        return (company.steelPrice || 650) / unit;
      }
      if (name.includes("cement")) {
        const unit = company.cementUnit > 0 ? company.cementUnit : 1;
        return (company.cementPrice || 350) / unit;
      }
    }
  ...
  const calcTransport = () => {
    ...
    items.forEach((it) => {
      ...
      const qty = n(it.itemQty);

      if (name.includes("steel")) {
        totalKg += qty;
      } else if (name.includes("cement")) {
        totalKg += qty * BAG_WEIGHT_KG;
      }
    });
    ...
    if (dist >= 1 && dist <= 4) ratePer1000kg = 650;
    else if (dist >= 5 && dist <= 10) ratePer1000kg = 700;
    else if (dist >= 10) ratePer1000kg = 850;

    return (totalKg * ratePer1000kg) / 1000;
  };

   const submit = () => {
  if (!customerName || !customerPhone) return Alert.alert("Error", "Enter customer name & phone");
  ...
  setOrders((prev) =>
    prev.map((o) =>
      o.id === orderToEdit.id
        ? { ...o, customerName, customerPhone, items, distance: n(distance), paymentMethod, transport, subtotal, finalTotal, createdAtMs: o.createdAtMs || Date.now(), status: o.status }
        : o
    )
  );
  saveUserOrder({
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.totals}>
        ...
        <Text>Transport: ₹{calcTransport().toFixed(2)}</Text>
        <Text style={{ fontWeight: "700" }}>
          Final Total: ₹{(orderTotal(items) + calcTransport()).toFixed(2)}
        </Text>
      </View>
