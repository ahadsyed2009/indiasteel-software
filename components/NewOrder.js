...
import { saveUserOrder, saveCustomer } from "../firebaseHelpers";
...
export default function NewOrder({ navigation, route }) {
  const { orders, setOrders, companies = [], customers, shopId } = useContext(OrderContext);
  ...
  // Add this function in NewOrder.js, before the submit function
const handlePhoneChange = (phone) => {
  setCustomerPhone(phone);

  // Auto-fill when phone matches an existing customer
  const match = (customers || []).find(c => c.customerPhone === phone);
  if (match) {
    setCustomerName(match.customerName || "");
    setPlace(match.place || "");
    setDriverName(match.driverName || "");
    setTransport(match.transport ? String(match.transport) : "");
  ...
  const submit = () => {
    ...
    const discountValue    = discountType === "%"
      ...
      : n(discount);
    const finalTotal       = Math.max(preDiscountTotal - discountValue, 0);

    // ✅ Save customer scoped to SHOP (not individual user)
    // Any staff member of this shop can see all customers
    if (shopId) {
      ...
      if (!existingCustomer) {
        ...
        const customerId = `${shopId}_${phoneKey}`; // keyed by shopId, not userId
        ...
        const customerData = {
          ...
          paymentMethod,
          shopId,                                    // required for fetchCustomers
          createdBy:     auth.currentUser?.uid || "unknown", // audit trail
          createdAtMs:   Date.now(),
        };

        saveCustomer(customerData).catch(err =>
    ...
    if (orderToEdit) {
      ...
      saveUserOrder(updatedOrder, shopId).catch(err =>
      ...
      saveUserOrder(newOrder, shopId).catch(err =>
        Alert.alert("Firebase Error", err.message)
      );
    }
              ...
              <View style={styles.stepWrapper}>
                <View style={[
                  ...
                  {isCompleted
                    ...
                    : <Text style={[styles.stepNumber, isActive && styles.activeStepNumber]}>{stepIndex}</Text>
                  }
                </View>
                <Text style={[styles.stepLabel, isActive && styles.activeLabel]}>{label}</Text>
              </View>
              {index < steps.length - 1 && (
                <View style={[styles.stepConnector, step > stepIndex && styles.completedConnector]} />
              )}
            </React.Fragment>
          );
        })}
  ...
  return (
    ...
    customerPhone={customerPhone} setCustomerPhone={handlePhoneChange}  // ✅ changed
    ...
    onNext={() => setStep(3)}     onBack={() => setStep(1)}
          ...
          {step === 3 && (
            <Step3
              ...
              place={place}                 transport={transport}
              driverName={driverName}       paymentMethod={paymentMethod}
...
const styles = StyleSheet.create({
  ...
  headerContainer:     { backgroundColor: "#FFFFFF", paddingHorizontal: 20, paddingTop: 20, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: "#E5E7EB" },
  headerSubtitle:      { fontSize: 12, color: "#6B7280" },
  stepperContainer:    { flexDirection: "row", alignItems: "center", paddingHorizontal: 20, paddingVertical: 20, backgroundColor: "#FFFFFF", borderBottomWidth: 1, borderBottomColor: "#E5E7EB" },
  stepWrapper:         { alignItems: "center", width: 60 },
  stepCircle:          { width: 36, height: 36, borderRadius: 18, backgroundColor: "#E5E7EB", alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "#E5E7EB", elevation: 2, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2 },
  activeCircle:        { backgroundColor: "#3B82F6", borderColor: "#3B82F6", elevation: 3, shadowOpacity: 0.2 },
  completedCircle:     { backgroundColor: "#10B981", borderColor: "#10B981", elevation: 3, shadowOpacity: 0.2 },
  stepNumber:          { fontSize: 15, fontWeight: "600", color: "#9CA3AF" },
  activeStepNumber:    { color: "#FFFFFF" },
  checkmark:           { fontSize: 16, color: "#FFFFFF", fontWeight: "bold" },
  stepLabel:           { marginTop: 6, fontSize: 11, fontWeight: "500", color: "#9CA3AF", textAlign: "center" },
  activeLabel:         { color: "#3B82F6", fontWeight: "600" },
  stepConnector:       { flex: 1, height: 2, backgroundColor: "#E5E7EB", alignSelf: "center", marginHorizontal: 2 },
