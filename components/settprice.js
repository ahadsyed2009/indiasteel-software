...
import { saveCompany, deleteCompany } from "../firebaseHelpers";

export default function SettPrice() {
  const { companies, setCompanies, shopId, userId } = useContext(OrderContext);
  ...
  // --- Add company ---
  const addCompany = () => {
    if (!userId) return Alert.alert("Error", "Not authenticated");
    if (!shopId) return Alert.alert("Error", "No shop found");
    ...
    let newCompany = {
      ...
      type,
      userId,
      shopId, // ✅ required for correct Firebase path
    };
    ...
    // ✅ passes full company object so saveCompany can use shopId
    saveCompany(newCompany).catch((err) => console.error("Error saving company:", err));
  };

  const resetForm = () => {
    setNewCompanyName("");
    setType("steel");
    setSteelDetails([{ diameter: "6", price: "", qty: "" }]);
    setCementPrice("");
    setCementQty("");
  };
    ...
    Alert.alert("Delete Company", `Are you sure you want to delete ${company.name}?`, [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: () => {
          ...
          // ✅ pass full company object (not just company.id) so deleteCompany has shopId
          deleteCompany(company).catch((err) => console.error("Error deleting company:", err));
        },
      },
    ]);
  };
  ...
  const saveEditCompany = () => {
    ...
    // ✅ editCompany already has shopId so this is safe
    saveCompany(editCompany).catch((err) => console.error("Error updating company:", err));
  ...
  const renderCompany = ({ item }) => (
    <View style={styles.companyCardContainer}>
      ...
      <View style={styles.actionButtons}>
        ...
        <TouchableOpacity style={styles.deleteBtn} onPress={() => handleDeleteCompany(item)}>
...
const styles = StyleSheet.create({
  ...
  formTitle: { fontSize: 20, fontWeight: "700", color: "#1f2937", marginLeft: 12 },
  inputGroup: { marginBottom: 16 },
  inputLabel: { fontSize: 14, fontWeight: "600", color: "#374151", marginBottom: 8 },
  inputLabeld: { fontSize: 17, fontWeight: "600", color: "#374151", marginBottom: 8 },
  input: {
  ...
  picker: { height: 50, flex: 1, color: "#1f2937" },
  groupBox: {
    backgroundColor: "#f9fafb",
    borderRadius: 16,
    padding: 16,
  ...
  groupBoxHeader: { flexDirection: "row", alignItems: "center", marginBottom: 16 },
  groupTitle: { fontSize: 16, fontWeight: "700", color: "#1f2937", marginLeft: 8 },
  steelRow: {
    backgroundColor: "#fff",
    borderRadius: 12,
    padding: 12,
  ...
  steelRowTitle: { fontSize: 14, fontWeight: "600", color: "#6b7280" },
  ...
  addRowBtnText: { color: "#007bff", fontWeight: "600", fontSize: 14, marginLeft: 8 },
  formActions: { flexDirection: "row", marginTop: 8, gap: 12 },
  addBtn: {
    flex: 1,
  ...
  companyCardContainer: { marginBottom: 16 },
  companyCard: {
    backgroundColor: "#fff",
    borderRadius: 16,
    padding: 18,
  ...
  companyName: { fontSize: 18, fontWeight: "700", color: "#1f2937" },
  companyType: { fontSize: 13, color: "#6b7280", marginTop: 2 },
  divider: { height: 1, backgroundColor: "#f3f4f6", marginBottom: 12 },
  priceSection: { marginBottom: 12 },
  sectionHeader: { flexDirection: "row", alignItems: "center", marginBottom: 8 },
  sectionTitle: { fontSize: 14, fontWeight: "600", color: "#6b7280", marginLeft: 6 },
  priceRow: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  ...
  diameterText: { fontSize: 13, fontWeight: "600", color: "#007bff" },
  priceValue: { fontSize: 15, fontWeight: "700", color: "#1f2937" },
  ...
  emptySubtitle: { fontSize: 16, color: "#9ca3af" },
});